...
# Cur Rev.3: ResNet18: batch_size=32, EPOCH=20, lr=0.0001, lr_step = 5, gamma=0.1, with aug trainset(transpose of vertical and transverse directions)
import datetime
import os
import time
import numpy as np
...
def dataset_and_labels(folder_address):
	pic_list = os.listdir(folder_address + '\\train_vt')
	...
	for i in range(pic_num):
		...
		pic_matrix[i] = img2matrix(folder_address + '\\train_vt\\%s' % file_name)
...
scheduler = lr_scheduler.StepLR(optimizer, lr_step, 0.1) # step_size=5, gamma=0.1
...
if __name__ == '__main__':
	...
	torch.save(model, 'model_ResNet18_vt_E20.pth')
	...
	df.to_csv('..\\submit\\submit_' + date_flag + '_ResNet18_vt_E20.csv', index=False)
