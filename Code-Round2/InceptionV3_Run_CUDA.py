...
# Cur Rev.4: InceptionV3_B16: EPOCH=6, lr=0.0001, lr_step = 2, gamma=0.1
...
def dataset_and_labels(folder_address):
	pic_list = os.listdir(folder_address + '\\train_299')
	...
	for i in range(pic_num):
		...
		pic_matrix[i] = img2matrix(folder_address + '\\train_299\\%s' % file_name)
...
test_loader = DataLoader(dataset = test_dataset, shuffle = False, batch_size = 32) # batch_size根据情况修改, shuffle=False!
...
if __name__ == '__main__':
	...
	torch.save(model, 'model_InceptionV3_E06.pth')
	...
	df.to_csv('InceptionV3_Preds_06.csv', index=False)
